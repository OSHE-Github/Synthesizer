...
const int led_pin = 31; // pin of indicator LED
...
bool voices_on[voices] = {false, false, false, false, false, false, false, false}; // which voices are currently being used by a held down note
...
int voice_notes[voices] = {0, 0, 0, 0, 0, 0, 0, 0}; // which note is currently assigned to each voice
int release_queue[voices] = {0, 0, 0, 0, 0, 0, 0, 0}; // FIFO of voices still in release stage
...
void setup(){ // code that runs once at bootup
  ...
  filter5.octaveControl(1);
  filter6.octaveControl(1);
  filter7.octaveControl(1);
  filter8.octaveControl(1);
...
void update_note(int voice){ // changes the note (frequency) that the oscillators of a given voice are playing
  switch(voice){
    case 0:
      ...
      osc1B.frequency(voice_freqs[0] * ((detune_pot * detune_inc) + 1)); // oscillator 2 can be detuned from oscillator 1
      break;
    case 1:
      osc2A.frequency(voice_freqs[1]);
      osc2B.frequency(voice_freqs[1] * ((detune_pot * detune_inc) + 1));
      break;
    case 2:
      osc3A.frequency(voice_freqs[2]);
      osc3B.frequency(voice_freqs[2] * ((detune_pot * detune_inc) + 1));
      break;
    case 3:
      osc4A.frequency(voice_freqs[3]);
      osc4B.frequency(voice_freqs[3] * ((detune_pot * detune_inc) + 1));
      break;
    case 4:
      osc5A.frequency(voice_freqs[4]);
      osc5B.frequency(voice_freqs[4] * ((detune_pot * detune_inc) + 1));
      break;
    case 5:
      osc6A.frequency(voice_freqs[5]);
      osc6B.frequency(voice_freqs[5] * ((detune_pot * detune_inc) + 1));
      break;
    case 6:
      osc7A.frequency(voice_freqs[6]);
      osc7B.frequency(voice_freqs[6] * ((detune_pot * detune_inc) + 1));
      break;
    case 7:
      osc8A.frequency(voice_freqs[7]);
      osc8B.frequency(voice_freqs[7] * ((detune_pot * detune_inc) + 1));
      break;
  }
}
